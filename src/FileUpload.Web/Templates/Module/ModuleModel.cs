...
public class ModuleModel
{
    ...
    public ModuleModel(String model, String controller, String? area)
    {
        ...
        Type modelType = assembly.GetType($"FileUpload.Objects.{model}") ?? typeof(AModel);
        Type viewType = assembly.GetType($"FileUpload.Objects.{model}View") ?? typeof(AView<>);
        BindingFlags declaredOnly = BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly;

        ControllerNamespace = $"FileUpload.Controllers.{area}".Trim('.');
        ...
        {
            Type type = assembly.GetType($"FileUpload.Objects.{model}{view}View") ?? viewType;
